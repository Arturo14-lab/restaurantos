...
const sections = [
  { icon: Building2, title: 'Empresa', text: 'Datos fiscales, identidad, moneda y zona horaria.', status: 'Configurada' },
  { icon: MapPin, title: 'Restaurantes', text: 'Locales, direcciones, horarios, zonas y almacenes.', status: '1 restaurante' },
...
export function SettingsHome() {
  return <div className="fade-in">
    ...
    <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4 mt-8">{sections.map(({icon:Icon,title,text,status})=><button key={title} className="card p-5 text-left hover:border-[#9caf9f] hover:-translate-y-0.5 transition-all">
      <div className="flex items-start justify-between gap-4"><span className="h-11 w-11 rounded-xl bg-[#e8efe9] text-[#315f4c] grid place-items-center"><Icon size={21}/></span><span className="text-xs font-semibold rounded-full bg-[#f0f3f0] text-[#6c7771] px-3 py-1">{status}</span></div>
      <h2 className="font-semibold mt-5">{title}</h2><p className="text-sm text-[#75807a] mt-2 leading-relaxed">{text}</p>
    </button>)}</div>
  </div>
}
